...
- **画像撮影機能** — 選択したウィンドウの画像を取得する。画像は実行ファイルzikkokey.pyと同じフォルダにファイル名shot.png保存される。Claudeには絶対パス（例：c:\zikkokey\shot.png）で伝えるのが確実だが、CLAUDE.mdに以下のように書いておけば、画像を見るように言えば伝わる。
